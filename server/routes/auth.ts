...
export const login: RequestHandler = async (req, res) => {
  ...
  try {
    const q = await pool.query(
      "SELECT id, name, email, role, division, is_active, password_hash FROM employees WHERE email=$1 LIMIT 1",
      [email],
    );
    if (!q.rows.length)
    ...
    try {
      const roleMapRes = await pool.query(
        "SELECT value FROM app_settings WHERE key='roleModuleMap'",
      );
      const userMapRes = await pool.query(
        "SELECT value FROM app_settings WHERE key='userModuleMap'",
      );
      const roleMap = roleMapRes.rows[0]?.value || {};
      const userMap = userMapRes.rows[0]?.value || {};
      if (userMap && userMap[u.id] && Array.isArray(userMap[u.id]))
        allowedModules = userMap[u.id];
      else if (roleMap && roleMap[u.role] && Array.isArray(roleMap[u.role]))
        allowedModules = roleMap[u.role];
    } catch {}
...
export const forgotPassword: RequestHandler = async (req, res) => {
  ...
  try {
    const q = await pool.query(
      "SELECT id, email, name FROM employees WHERE email=$1 LIMIT 1",
      [email],
    );
    if (!q.rows.length) {
    ...
    await pool.query(
      "INSERT INTO password_resets(id, user_id, token, expires_at, used) VALUES ($1,$2,$3,$4,$5)",
      [id, u.id, token, expires.toISOString(), false],
    );
...
export const resetPassword: RequestHandler = async (req, res) => {
  ...
  try {
    ...
    await pool.query("UPDATE employees SET password_hash=$1 WHERE id=$2", [
      hash,
      rec.user_id,
    ]);
