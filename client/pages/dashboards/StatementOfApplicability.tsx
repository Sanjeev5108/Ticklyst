...
// Lightweight view of Framework node details used for read-only display in SoA
interface FrameworkNodeDetails {
  process_id?: string;
  process_name?: string;
...
export default function StatementOfApplicability() {
  ...
  const [clientNeedsIndustryMapping, setClientNeedsIndustryMapping] = useState<
    string | null
  >(null);

  const [tree, setTree] = useState<TreeNode[]>([]);
  const [frameworkDetailsById, setFrameworkDetailsById] = useState<
    Record<string, FrameworkNodeDetails>
  >({});
  const [processOptions, setProcessOptions] = useState<
    { id: string; name: string }[]
  >([]);
  const [selectedProcessesIndustry, setSelectedProcessesIndustry] = useState<
    string[]
  >([]);
  const [isSaving, setIsSaving] = useState(false);
  const [selectedProcessesClient, setSelectedProcessesClient] = useState<
    string[]
  >([]);
  const [industryProcessMap, setIndustryProcessMap] = useState<
    Record<string, string[]>
  >({});
  const [industryNodeMap, setIndustryNodeMap] = useState<
    Record<string, string[]>
  >({});
  ...
  const [clientSelections, setClientSelections] = useState<Set<string>>(
    new Set(),
  );
  ...
  const allFields = [
    ...
    "Risk",
    "Control",
    "Risk Related Departments",
    "Controls Related Departments",
    "Risk Category",
    "Control Type",
    "Reference",
    "Applicability",
    ...
    (async () => {
      try {
        const res = await fetch(
          `/api/settings/${encodeURIComponent("soa:industry:" + selectedIndustry)}`,
        );
        const saved = res.ok
          ? await res.json()
          : LS.get(`soa:industry:${selectedIndustry}`);
        if (saved && typeof saved === "object") {
          if (Array.isArray(saved.processes))
            setSelectedProcessesIndustry(saved.processes);
          if (
            ...
            setIndustrySelections(
              new Set(Object.keys(appMap).filter((id) => appMap[id] === true)),
            );
          }
        }
      } catch {
        ...
        if (
          saved &&
          typeof saved === "object" &&
          Array.isArray((saved as any).processes)
        ) {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        const res = await fetch(
          `/api/settings/${encodeURIComponent("soa:industry:" + ind)}`,
        );
        const saved = res.ok ? await res.json() : LS.get(`soa:industry:${ind}`);
        if (
          !saved ||
          !Array.isArray(saved.processes) ||
          !saved.processes.length
        ) {
        ...
        try {
          const rc = await fetch(
            `/api/settings/${encodeURIComponent("soa:client:" + selectedClientId)}`,
          );
          if (rc.ok) savedClient = await rc.json();
          else savedClient = LS.get(`soa:client:${selectedClientId}`);
        } catch {
        ...
        if (savedClient && typeof savedClient === "object") {
          if (
            Array.isArray(savedClient.processes) &&
            savedClient.processes.length
          )
            setSelectedProcessesClient(savedClient.processes);
          if (
            savedClient.nodeApplicability &&
            typeof savedClient.nodeApplicability === "object"
          ) {
            ...
            setClientSelections(
              new Set(
                Object.keys(appMapClient).filter(
                  (id) => appMapClient[id] === true,
                ),
              ),
            );
            return;
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const mapped: TreeNode[] = nodes.map((n: any) => ({
          id: n.id,
          type: n.type,
          name: n.name,
          parentId: n.parentId,
        ...
        setTree(sorted);
        const details = (data?.detailsById || {}) as Record<
          string,
          FrameworkNodeDetails
        >;
        setFrameworkDetailsById(details);
        setProcessOptions(
          sorted
  ...
  const getHierarchyForNode = (node: TreeNode | null) => {
    const result: {
      process?: TreeNode;
      subprocess?: TreeNode;
      activity?: TreeNode;
    ...
    let current: TreeNode | undefined | null = node;
    while (current) {
      if (current.type === "process") result.process = current;
      else if (current.type === "subprocess") result.subprocess = current;
      else if (current.type === "activity") result.activity = current;
      else if (current.type === "risk") result.risk = current;
      else if (current.type === "control") result.control = current;
      current = current.parentId
        ? tree.find((n) => n.id === current!.parentId) || undefined
        : undefined;
    }
    return result;
  };
  ...
  const activeDetails: Record<string, NodeDetails> =
    tab === "industry"
  ...
  const toggleSelectCascade = (id: string, checked: boolean) => {
    ...
    setTree((prev) =>
      prev.map((n) => (setIds.has(n.id) ? { ...n, isSelected: checked } : n)),
    );
    if (tab === "industry") {
      setDetailsIndustry((prev) => {
        const current = prev[selectedIndustry] || {};
    ...
    if (tab === "industry") {
      setIndustrySelections((prev) => {
        const next = new Set(prev);
        for (const nid of ids) {
          checked ? next.add(nid) : next.delete(nid);
        }
        return next;
      });
    } else {
  ...
  const selectedNode = selectedNodeId
    ...
    : null;
  const nodeDetails: NodeDetails | undefined = selectedNodeId
    ? (activeDetails as any)[selectedNodeId!]
    : undefined;
  const frameworkDetailsForSelected: FrameworkNodeDetails | undefined =
    selectedNode && frameworkDetailsById[selectedNode.id]
      ? frameworkDetailsById[selectedNode.id]
      : undefined;
  const hierarchyForSelected = selectedNode
    ? getHierarchyForNode(selectedNode)
    : null;
  ...
  const updateNodeDetails = (patch: Partial<NodeDetails>) => {
    ...
    if (tab === "industry") {
      setDetailsIndustry((prev) => {
        const current = prev[selectedIndustry] || {};
        const base = current[selectedNodeId] || {
          description: "",
          industry: selectedIndustry,
          client: "",
          itemId: "",
  ...
  const resetSelectionsToFilter = () => {
    if (tab === "industry") {
      setDetailsIndustry((prev) => {
        const cur = { ...(prev[selectedIndustry] || {}) } as Record<
          string,
          NodeDetails
        >;
        Object.keys(cur).forEach((k) => {
          if (!cur[k].industry) cur[k].industry = selectedIndustry;
        });
        return { ...prev, [selectedIndustry]: cur };
      });
    } else {
      setDetailsClient((prev) => {
        const cur = { ...(prev[selectedClientId] || {}) } as Record<
          string,
          NodeDetails
        >;
        Object.keys(cur).forEach((k) => {
          if (!cur[k].client) cur[k].client = selectedClientId;
        });
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(v: any) => setFilterApplicability(v)}
                  >
            ...
            onClick={() => {
              const rowsBase = renderNodes
                .filter((n) => {
                  ...
                  return filterApplicability === "all"
                    ? true
                    : filterApplicability === "app"
                      ? app === true
                      : filterApplicability === "na"
                        ? app === false
                ...
                .map((n) => {
                  ...
                  while (cur) {
                    if (cur.type === "process") path[0] = cur.name;
                      ...
                      riskName = riskName || cur.name;
                    else if (cur.type === "control")
                      controlName = controlName || cur.name;
                    cur = cur.parentId
                      ? tree.find((x) => x.id === cur!.parentId) || undefined
                      : undefined;
                  }
                  ...
                  if (selectedFields.includes("Process"))
                    row["Process"] = path[0] || "";
                  if (selectedFields.includes("Subprocess"))
                    row["Subprocess"] = path[1] || "";
                  if (selectedFields.includes("Activity"))
                    row["Activity"] = path[2] || "";
                  if (selectedFields.includes("Risk"))
                    row["Risk"] = riskName || "";
                  if (selectedFields.includes("Control"))
                    row["Control"] = controlName || "";
                  if (selectedFields.includes("Risk Related Departments"))
                    row["risk related departments"] = "";
                  if (selectedFields.includes("Controls Related Departments"))
                    row["controls related departments"] = "";
                  if (selectedFields.includes("Risk Category"))
                    row["Risk Category"] = "";
                  if (selectedFields.includes("Control Type"))
                    row["Control type"] = "";
                  if (selectedFields.includes("Reference"))
                    row["Reference"] = n.id;
                  if (selectedFields.includes("Applicability"))
                    row["Applicability"] =
                      app === true
                        ? "Applicable"
                        : app === false
                          ? "Not Applicable"
                          : "Undecided";
                  if (selectedFields.includes("Industry"))
                    row["Industry"] =
                      tab === "industry"
                        ? selectedIndustry
                  ...
                  if (selectedFields.includes("Client"))
                    row["Client"] =
                      tab === "client" ? selectedClient?.name || "" : "";
                  return row;
                });
              let rows: any[] = [];
        ...
        <CardContent className="p-6 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2 text-sm">
              <Button
                variant={tab === "industry" ? "default" : "outline"}
                size="sm"
          ...
          {tab === "industry" ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="mt-1">
                  <IndustrySelect
                    ...
                    onChange={(v) => {
                      ...
                      setSelectedProcessesIndustry(industryProcessMap[v] || []);
                      setIndustrySelections(new Set());
                    }}
              ...
              <div>
                ...
                <ProcessesMultiSelect
                  ...
                  value={selectedProcessesIndustry}
                  onChange={setSelectedProcessesIndustry}
                />
              </div>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <Label>Processes (Multiple)</Label>
                <ProcessesMultiSelect
                  options={processOptions.filter((p) =>
                    industryProcessMap[selectedClient?.industry || ""]?.length
                      ? industryProcessMap[selectedClient!.industry]!.includes(
                          p.id,
                        )
                      : false,
                  )}
                ...
                {tab === "industry" ? (
                  <Button
                    ...
                    onClick={async () => {
                      try {
                        setIsSaving(true);
                        setIndustryProcessMap((prev) => ({
                          ...prev,
                          [selectedIndustry]: selectedProcessesIndustry,
                        }));
                        setIndustryNodeMap((prev) => ({
                          ...prev,
                          [selectedIndustry]: Array.from(industrySelections),
                        }));
                        const nodeApplicability: Record<
                          string,
                          boolean | null
                        > = {};
                      ...
                      try {
                        ...
                        try {
                          ...
                          const idToName: Record<string, string> =
                            Object.fromEntries(
                              processOptions.map((p) => [p.id, p.name]),
                            );
                          existing[selectedClient?.name || selectedClientId] = (
                            selectedProcessesClient || []
                          )
                            .map((id) => idToName[id])
                            .filter(Boolean);
                          try {
                            await fetch(`/api/settings/${nameKey}`, {
                              method: "POST",
                              headers: { "Content-Type": "application/json" },
              ...
              <div className="divide-y pb-6">
                {renderNodes.map((node) => {
                  ...
                  return (
                    <div
                      key={node.id}
                      className={`flex items-center py-2 px-1 ${selectedNodeId === node.id ? "bg-blue-50" : ""}`}
                    >
                      <div
                      ...
                      >
                          ...
                          onClick={() => {
                            ...
                            if (tab === "industry") {
                              setDetailsIndustry((prev) => {
                                ...
                                const nextMap: Record<string, NodeDetails> = {
                                  ...cur,
                                };
                                for (const id of ids) {
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2">
                            <div
                              className={`text-xs capitalize ${getTypeColorClass(node.type)} truncate`}
                            >
                              {node.type}
                            </div>
                            <span
                              className={`text-[10px] px-1.5 py-0.5 rounded-full ${nd?.applicable === true ? "bg-green-100 text-green-700" : nd?.applicable === false ? "bg-red-100 text-red-700" : "bg-gray-100 text-gray-700"}`}
                            >
                              {nd?.applicable === true
                                ? "Applicable"
                                : nd?.applicable === false
